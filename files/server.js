...
// ════════════════════════════════════════════════════════════════════════
// ║  TARIFAS (Rates) — Lodgify v2 · lectura + escritura por alojamiento    ║
// ╚════════════════════════════════════════════════════════════════════════

// Cache: houseId → [{ id, name, room_type_id }] · 10 min TTL
const _lodgifyRoomsCache = new Map();

// GET /lodgify-rooms?houseId=X — trae los room types de una propiedad
app.get("/lodgify-rooms", async (req, res) => {
  try {
    ...
    const cached = _lodgifyRoomsCache.get(houseId);
    if (cached && (Date.now() - cached.ts) < 600_000) return res.json({ ok:true, rooms: cached.rooms, cached:true });
    const url = `https://api.lodgify.com/v2/properties/${encodeURIComponent(houseId)}/rooms`;
    const r = await fetch(url, { headers: { "X-ApiKey": apiKey, accept:"application/json" }});
    if (!r.ok) {
      const txt = await r.text().catch(()=>'');
      return res.status(r.status).json({ ok:false, error:`Lodgify HTTP ${r.status}`, detail: txt.slice(0,500) });
    }
    const data = await r.json();
    const rooms = Array.isArray(data) ? data : (data.items || data.rooms || []);
    _lodgifyRoomsCache.set(houseId, { rooms, ts: Date.now() });
...
// GET /lodgify-rates?houseId=X&roomTypeId=Y&from=YYYY-MM-DD&to=YYYY-MM-DD
// Devuelve el calendario de tarifas por noche.
app.get("/lodgify-rates", async (req, res) => {
  try {
    const apiKey = process.env.LODGIFY_API_KEY;
    if (!apiKey) return res.status(500).json({ ok:false, error:"LODGIFY_API_KEY no configurada" });
    const houseId = String(req.query.houseId || "").trim();
    ...
    if (!houseId || !roomTypeId || !from || !to) {
      return res.status(400).json({ ok:false, error:"houseId, roomTypeId, from, to son requeridos" });
    }
    ...
    const url = `https://api.lodgify.com/v2/rates/calendar?${params.toString()}`;
    const r = await fetch(url, { headers: { "X-ApiKey": apiKey, accept:"application/json" }});
    if (!r.ok) {
      const txt = await r.text().catch(()=>'');
      return res.status(r.status).json({ ok:false, error:`Lodgify HTTP ${r.status}`, detail: txt.slice(0,500) });
    ...
    const data = await r.json();
    res.json({ ok:true, houseId, roomTypeId, from, to, data });
  } catch (e) {
    res.status(500).json({ ok:false, error: String(e?.message || e) });
  }
});

// POST /lodgify-save-rates — guarda tarifas por noche.
// Body: { houseId, roomTypeId, rates: [{from:'YYYY-MM-DD', to:'YYYY-MM-DD', pricePerDay:Number, minStay?, maxStay?, pricePerAdditionalGuest?}] }
app.post("/lodgify-save-rates", async (req, res) => {
  try {
    const apiKey = process.env.LODGIFY_API_KEY;
    if (!apiKey) return res.status(500).json({ ok:false, error:"LODGIFY_API_KEY no configurada" });
    const { houseId, roomTypeId, rates } = req.body || {};
    if (!houseId || !roomTypeId || !Array.isArray(rates) || !rates.length) {
      return res.status(400).json({ ok:false, error:"houseId, roomTypeId, rates[] son requeridos" });
    }
    // Payload Lodgify v2: property_id, room_type_id, rates:[{start_date,end_date,price_per_day,min_stay,max_stay,price_per_additional_guest}]
    const payload = {
      property_id: Number(houseId),
      room_type_id: Number(roomTypeId),
      rates: rates.map(r => ({
        is_default: false,
        start_date: r.from,
        end_date: r.to,
        price_per_day: Number(r.pricePerDay) || 0,
        min_stay: Number(r.minStay) || 1,
        max_stay: Number(r.maxStay) || 0,
        price_per_additional_guest: Number(r.pricePerAdditionalGuest) || 0,
      })),
    };
    const url = `https://api.lodgify.com/v2/rates/savingRates`;
    ...
    const txt = await r.text().catch(()=>'');
    if (!r.ok) return res.status(r.status).json({ ok:false, error:`Lodgify HTTP ${r.status}`, detail: txt.slice(0,800), payload });
    let data;
    try { data = JSON.parse(txt); } catch { data = { raw: txt }; }
    res.json({ ok:true, saved: rates.length, data });
  } catch (e) {
    res.status(500).json({ ok:false, error: String(e?.message || e) });
  }
});
